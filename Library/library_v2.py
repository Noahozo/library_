...
class BookManager:
    ...
    def update_book(self):
        ...
        try:
            with connection.cursor() as cursor:
                # 建立 SQL 語句
                if new_name != '':
                    sql1 = 'UPDATE books SET name="%s" where name="%s";'\
                        % (new_name,bname)
                    cursor.execute(sql1)
                if new_author != '':
                    sql2 = 'UPDATE books SET author="%s" where author="%s";'\
                        % (new_author,bauthor)
                    cursor.execute(sql2)
                if new_year != '':
                    sql3 = 'UPDATE books SET year="%s" where year="%s";'\
                        % (new_year,byear)  
                    cursor.execute(sql3)
                    print(sql3)  
                # 執行 SQL 語句
    ...
    def manu(self):
        ...
        while True:
         ...
         '''))
            if choice ==0:
                self.select_book() # 顯示書庫 ok
                #顯示每本書的資訊
            elif choice ==1:
                self.add_book() # 新增書本 ok
            elif choice ==2:
                self.delete_book() # 刪除書本 ok
            elif choice ==3:
                self.update_book() # 更新書庫 ok
            elif choice ==4:
                self.add_review() # 新增心得 ok
            elif choice ==5:
                self.res_review() #修改心得 ok
            elif choice ==6:
                self.select_author() # 搜尋作者作品 ok
            elif choice ==7:
